...
export class TenderPage implements OnInit {
  ...
  loadTenders(userType: string = '') {
    ...
    this.loading = true;
    let apiUrl = 'http://localhost:8080/api/tenders';
    if (userType) {
      apiUrl += `?userType=${userType}`;
    }
    this.http.get<TenderData[]>(apiUrl).subscribe({
      next: (data) => {
  ...
  get filteredTenders(): TenderData[] {
    return this.tenders.filter(tender => {
      ...
      const matchesUserType = this.userTypeFilter === '' || 
                             (tender.userType && tender.userType.toUpperCase() === this.userTypeFilter.toUpperCase());
  ...
  applyUserTypeFilter(): void {
    // Use client-side filtering - no need to call API
    console.log('Applying userType filter:', this.userTypeFilter);
    this.currentPage = 1;
    this.cdr.detectChanges();
  }
}
